...
flash_get_geom (flash_info_t *flash_info)
{
    ...
    flash_info->size = CONFIG_SYS_FLASH_SIZE; /* bytes */
    flash_info->sector_count = flash_info->size/CONFIG_SYS_FLASH_SECTOR_SIZE;

    for (i = 0; i < flash_info->sector_count; i++) {
        flash_info->start[i] = CONFIG_SYS_FLASH_BASE + (i * CONFIG_SYS_FLASH_SECTOR_SIZE);
