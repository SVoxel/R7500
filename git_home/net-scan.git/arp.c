...
static struct in_addr quantenna;
...
int update_arp_table(uint8 *mac, struct in_addr ip, int isrepl)
{
	...
	if (memcmp(&quantenna, &ip, sizeof(struct in_addr)) == 0) {
		printf("ignore quantenna device");
		return 0;
	}
	...
	for (i = 0; i < (NEIGH_HASHMASK + 1); i++) {
		for (pprev = &arp_tbl[i], u = *pprev; u; ) {
			if (u->active == 0) {
				...
				system("wlan stainfo > /tmp/ABC_tmpmac");
