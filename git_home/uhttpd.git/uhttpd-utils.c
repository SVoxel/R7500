...
{
        if(strcasecmp(httphost, "routerlogin.net")==0 || strcasecmp(httphost, "www.routerlogin.net")==0)
                return 0;
...
void update_login(void)
{
	...
	if ( readw(NO_NEEDAUTH) != 1 )
	{
		writew(NO_NEEDAUTH, 1);
	}
	else if ( *login_ip == '\0')
	{
...
{
	int i;
	char *p, *authorization;
	char authinfo[128],http_auth[128];
	...
	i = uh_b64decode(authinfo, sizeof(authinfo)-1, authorization + 6, strlen(authorization) - 6);
	authinfo[i] = '\0';

	p = config_get("http_username");
	if (*p == '\0')
		return;
	i = sprintf(http_auth, "%s:", p);

	p = config_get("http_passwd");
	if (*p == '\0')
		return;

	sprintf(&http_auth[i], "%s", p);

	if (strcmp(http_auth, authinfo) == 0)
	{
		update_login();
	}
}
